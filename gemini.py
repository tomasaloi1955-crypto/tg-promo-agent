...
Бесплатный тест: 3 пробных поста в стиле канала до оплаты. Цены и ссылку владелица пришлёт
сама, если человек ответит, — в первом сообщении их быть не должно.
...
- message — если fit=true: ПЕРВОЕ сообщение незнакомому владельцу канала на русском, на «Вы».
  Главное — чтобы его не отметили как спам. Поэтому:
  • коротко: 3–4 предложения, до 400 знаков;
  • БЕЗ ссылок, БЕЗ @упоминаний, БЕЗ цен и цифр, БЕЗ хэштегов и списков, максимум 1 эмодзи;
  • начни с «Ассаляму алейкум!», если muslim=true, иначе с «Здравствуйте!»;
  • сразу одна конкретная живая деталь именно этого канала (что продают/чему учат/о чём пишут),
    чтобы было видно, что пишет человек, который канал смотрел, а не рассылка;
  • суть одной фразой: могу сделать так, чтобы посты в канале выходили каждый день в его
    стиле, а от владельца — 10 минут в неделю;
  • закончи вопросом-разрешением, например: «Можно расскажу подробнее?» или «Интересно
    было бы посмотреть 3 пробных поста для вашего канала — бесплатно?»;
  • формулировки каждый раз свои, без шаблонных фраз вроде «уникальное предложение»,
    «выгодно», «акция», «только сегодня»; без давления и без выдуманных фактов; без подписи.
  Если fit=false — пустая строка.
  Пишет ЖЕНЩИНА: все слова о себе — только в женском роде («посмотрела», «увидела», «рада»,
  «готова», «сделала», «могла бы»), никогда «посмотрел», «рад», «готов». Если сомневаешься —
  перестрой фразу без прошедшего времени («смотрю ваш канал», «могу подготовить»)."""


def evaluate_channel_lead(title: str, about: str, posts: list[str]) -> Optional[dict]:
